...
if (!TOKEN) throw new Error('VITE_GH_TOKEN não definido (GraphQL requer token)');
...
export async function fetchFeaturedProjects(max = 6): Promise<Project[]> {
  const data = await graphQL<{ user: { pinnedItems: { nodes: RepoNode[] } } }>(QUERY_PINNED, {
    login: USERNAME,
    first: max
  });
...
export async function fetchAllProjects(options?: {
  ...
  while (hasNext && acc.length < maxRepos) {
    ...
    acc.push(...page.nodes.filter((r) => (options?.includeArchived ? true : !r.isArchived)));
...
export async function getProjectsSplit() {
  const [featured, all] = await Promise.all([fetchFeaturedProjects(), fetchAllProjects()]);
