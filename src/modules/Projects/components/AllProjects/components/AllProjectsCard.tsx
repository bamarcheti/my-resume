...
export const AllProjectsCard: React.FC<Props> = ({
  ...
  return (
    <div
      className={`${className} flex flex-col justify-between space-y-2 rounded-md bg-butterflyBush-600 dark:bg-stone-800 p-4 md:p-5 gap-4 w-full h-full`}
    >
      <h1>
        <a
          href={html_url}
      ...
      <ul className="flex flex-wrap items-center gap-1">
        {topics.map((topic, index) => (
          <li key={index}>
            ...
            <p className="flex items-center w-full cursor-default font-medium md:font-semibold xl:text-sm text-xs 2xl:text-base text-purple-600 dark:text-white bg-white dark:bg-purple-400 px-[3.8px] min-[375px]:px-[10px] md:px-2 rounded-md">
              {topic}
            </p>
          </li>
        ))}
